...
export class SearchService {
  ...
  async search(
    ...
    try {
      ...
      const indices = type === 'all' ? ['albums', 'tracks', 'playlists', 'users'] : [type];
          ...
          bool: {
            should: [
              // 정확한 매칭 (높은 점수)
              {
                multi_match: {
                  query,
                  fields: ['title^3', 'title.keyword^4', 'description^2', 'name^2'],
                  type: 'phrase',
                  boost: 2
                }
              },
  ...
  async indexDocument(
    ...
    try {
      ...
      throw new InternalServerErrorException('검색 인덱스 업데이트에 실패했습니다');
    }
  }
