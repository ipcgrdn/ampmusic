...
export function UserTagInput({ value, onChange, disabled }: UserTagInputProps) {
      ...
      try {
        ...
        const users = await Promise.all(
          newUserIds.map(async (id) => {
            const results = await searchUsers(id);
            return results[0];
          })
        );
        
        setSelectedUsers(prev => [...prev, ...users.filter(Boolean)] as User[]);
      } catch (error) {
      ...
      <Popover open={open} onOpenChange={handleOpenChange}>
        <PopoverTrigger asChild>
          ...
          >
            ...
            <IconChevronDown className="w-4 h-4 shrink-0 text-white/40 
              group-hover:text-white/60 transition-transform duration-200
